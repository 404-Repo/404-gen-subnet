...
Two duel artifacts per audited miner — both saved as MatchReports with kind='audit':
  ...
  Submitted audit — `audit_submitted.json` — submitted vs generated.
  ...
  Defender audit — `audit_{defender[:10]}.json` — generated vs the leader the miner beat.
       Verifies the solution genuinely beats the leader (quality).
       LEFT=defender hotkey, RIGHT=audited hotkey, RIGHT win = legit.

Margins are indexed in `audit_matrix.csv`; verdict computation in `round_runner` reads
the matrix in memory rather than the per-duel reports. Full reports are kept on disk
for forensics only.
"""
...
async def produce_generated_vs_submitted_audit(
    ...
    """Run submitted vs generated. Saved at rounds/{n}/{hotkey}/audit_submitted.json."""
    ...
    if shutdown.should_stop:
        logger.info(f"submitted duel interrupted before save: {hotkey[:10]}")
        return report
    await save_match_report(git_batcher, round_num, report, kind="audit")
    logger.info(f"submitted duel saved: {hotkey[:10]} submitted vs generated: margin={report.margin:+.2%}")
    return report
...
async def produce_generated_vs_defender_audit(
    ...
    """Run generated vs defender. Saved at rounds/{n}/{audited}/audit_{defender[:10]}.json.
    ...
    `latest_defender` for which no defender duel has been produced yet).
    """
    report = await run_match(
        openai=openai,
        prompts=prompts,
    ...
    if shutdown.should_stop:
        logger.info(f"defender duel interrupted before save: {audited_hotkey[:10]} vs {defender_hotkey[:10]}")
        return report
    await save_match_report(git_batcher, round_num, report, kind="audit")
    logger.info(
        f"defender duel saved: {audited_hotkey[:10]} generated vs {defender_hotkey[:10]}: "
